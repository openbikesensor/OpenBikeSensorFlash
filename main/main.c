...
// TODO: Can be shortened, no need for 4k of binary most id 0xff
// extern const uint8_t partition_table[] asm("_binary_sd_partition_table_bin_start");
const uint8_t partition_table[] = {
  ,
...
static void ensureOta0() {
    const esp_partition_t *running = esp_ota_get_running_partition();
    ESP_LOGI(TAG, "Running partition: %s", running->label);

    if (running->type == ESP_PARTITION_TYPE_APP && running->subtype != ESP_PARTITION_SUBTYPE_APP_OTA_0) {
        ...
        ESP_LOGI(TAG, "Copying ota%d (%s) -> ota0 (%s)", running->subtype - ESP_PARTITION_SUBTYPE_APP_OTA_0,
                 running->label, ota0->label);
        ESP_LOGI(TAG, "Target partition: size 0x%0x", ota0->size);
        // TODO: Check size; we get error if size differs!
        const uint16_t s = 8192;
        void *buffer = malloc(s);
        for (int i = 0; i < ota0->size; i += s) {
            ESP_ERROR_CHECK(esp_partition_read(running, i, buffer, s));
            ESP_ERROR_CHECK(esp_partition_write(ota0, i, buffer, s));
        }
        free(buffer);
...
void write_partition_table() {
    ...
    ESP_ERROR_CHECK(spi_flash_write(CONFIG_PARTITION_TABLE_OFFSET, partition_table, 0x2000));
    ESP_LOGI(TAG, "New partition table created, will reboot.");
    esp_restart();
}

const esp_partition_t *ensureNewPartitionTable() {
    const esp_partition_t *app = esp_partition_find_first(ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_OTA_1,
                                                          "app"); // already there?
    if (app) {
